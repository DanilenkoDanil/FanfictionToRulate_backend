...
but_status = KeyboardButton('Статус')
kb_cancel = ReplyKeyboardMarkup(resize_keyboard=True)
kb_cancel.add(but_cancel)
kb_main = ReplyKeyboardMarkup(resize_keyboard=True)
kb_main.row(but_parser, but_translate).row(but_download, but_status)
...
@dp.message_handler(commands=['Перевести', 'Скачать', 'Статус'], state=TranslateFSM.choose_book)
...
async def choose_book(message: types.Message, state: FSMContext):
    await message.reply('Выбери книгу', reply_markup=kb_cancel)  # reply_markup=kb_cancel
    async with state.proxy() as data:
        data['status'] = message.text
    # books_lst = get_books_list()
    list_of_books = ''
    for i in books_lst:
...
@dp.message_handler(commands='Спарсить', state=None)
